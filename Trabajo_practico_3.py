...
def Deteccion_texto(Imagen_BW, Prom, proporcion):
    Ancho = 0;  Alto = 1;   dil = 0;     er = 1

    # Dilatacion
    Ancho_kernel = int( Prom[Ancho]*proporcion[dil] )
    Alto_kernel = int( Prom[Alto]*proporcion[dil] )

    kernel = np.ones((Ancho_kernel,Alto_kernel),np.uint8)
    Deteccion = cv2.dilate(Imagen_BW, kernel, iterations = 1)   # Dilatacion

    # Llenar huecos
    imfill(Imagen_BW)

    # Erosion
    Ancho_kernel = int( Prom[Ancho]*proporcion[er] )
    Alto_kernel = int( Prom[Alto]*proporcion[er] )

    kernel = np.ones((Ancho_kernel,Alto_kernel),np.uint8)
    Deteccion = cv2.dilate(Imagen_BW, kernel, iterations = 1)   # Erosion

    # Deteccion = cv2.morphologyEx(Imagen_BW, cv2.MORPH_CLOSE, kernel) # Clausura
...
## --------------------------------------------------------------------- #
Img_palabra = Deteccion_texto(Img_BW, Prom, palabra)
imshow(Img_palabra, title = 'Palabras detectadas')

Img_parrafo = Deteccion_texto(Img_BW, Prom, parrafo)
